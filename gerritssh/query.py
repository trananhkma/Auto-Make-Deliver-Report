    ...
    def execute_on(self, the_site):
        ...
        def partial_query():
            ...
            raw = the_site.execute(' '.join(['query', limit,
                                             self.__query,
                                             standard_flags]))
            stat = self.stat_from_json(raw[0])
            lines = self.text_to_json(raw[1])
            return ([review.Review(l) for l in lines[:-1]]
                    if len(lines) > 1 else []), stat

        while self.__max_results == 0 and mores:
